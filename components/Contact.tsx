  ...
  return (
    <section id="contact" className="bg-accent-deep">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-6 py-24 text-center md:px-10">
        ...
        <h2 className="max-w-2xl font-heading text-4xl font-semibold leading-tight text-surface">
          {t.contact.heading}
        </h2>
        <p className="max-w-xl text-lg leading-relaxed text-[#d8d1c2]">{t.contact.sub}</p>
        <a
          href={`mailto:${t.contact.email}`}
          className="mt-2 inline-flex items-center gap-2.5 rounded-full bg-surface px-7 py-4 text-base font-semibold text-accent-deep transition-opacity hover:opacity-90"
        >
        ...
        <div className="flex items-center gap-7 pt-2">
          {t.contact.socials.map((s) => (
            <a
              ...
              className="inline-flex items-center gap-2 text-[15px] text-[#d8d1c2] transition-colors hover:text-surface"
            >
              {s.label === "LinkedIn" ? (
